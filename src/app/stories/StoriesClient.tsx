      ...
      <div className="absolute inset-0">
        {!imgFailed && story.image ? (
          <Image
            ...
            className="w-full h-full object-cover opacity-60 brightness-[1.8] group-hover:opacity-40 transition-all duration-700 group-hover:scale-110"
          />
        ) : (
...
export default function StoriesClient({ initialStories }: { initialStories: any[] }) {
  ...
  const visibleStories = initialStories.slice(0, visibleCount);
  ...
  return (
    <main className="min-h-screen pt-32 pb-20 px-6 md:px-12 max-w-7xl mx-auto">
          ...
          <button
            onClick={() =>
              setVisibleCount((prev) =>
                Math.min(prev + 6, initialStories.length)
              )
            }
      ...
      <p className="text-center text-gray-600 text-sm mt-6">
        Showing {visibleStories.length} of {initialStories.length} stories
      </p>
