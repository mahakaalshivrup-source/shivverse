...
export async function GET() {
  try {
    ...
    const mantras = metadata.mantras.map((mantra: any) => {
      const audioUrl = mantra.audio_filename ? `${cdn}/${mantra.audio_filename}` : "";
      const coverUrl = mantra.cover_filename ? `${cdn}/${mantra.cover_filename}` : "";
    ...
    const scriptures = metadata.scriptures.map((book: any) => {
      const pdfUrl = book.pdf_filename ? `${cdn}/${book.pdf_filename}` : "";
