...
export async function GET() {
  try {
    ...
    const stories = metadata.stories.map((story: any) => {
      const imageUrl = story.thumbnail_filename ? `${cdn}/${encodeURI(story.thumbnail_filename)}` : "";
      return {
        id: story.id,
        title: story.title,
        source: story.source,
    ...
    const mantras = metadata.mantras.map((mantra: any) => {
      const audioUrl = mantra.audio_filename ? `${cdn}/${encodeURI(mantra.audio_filename)}` : "";
      const coverUrl = mantra.cover_filename ? `${cdn}/${encodeURI(mantra.cover_filename)}` : "";
    ...
    const scriptures = metadata.scriptures.map((book: any) => {
      const pdfUrl = book.pdf_filename ? `${cdn}/${encodeURI(book.pdf_filename)}` : "";
      const thumbUrl = book.thumbnail_filename ? `${cdn}/${encodeURI(book.thumbnail_filename)}` : "";
      return {
        id: book.id,
        title: book.title,
        pdfUrl: pdfUrl,
